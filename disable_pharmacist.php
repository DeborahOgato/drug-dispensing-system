...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'Pharmacist') {
    ...
    $stmt = $conn->prepare("SELECT u.*, d.* FROM users u JOIN pharmacist ph ON u.user_id = ph.user_id WHERE u.user_id = ?");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        // Disable pharmacist's account
        if (isset($_POST['disable_pharmacist'])) {
            $update_stmt = $conn->prepare("UPDATE pharmacists SET is_disabled = 1 WHERE user_id = ?");
        ...
        // Enable pharmacist's account (Admin approval required)
        if (isset($_POST['enable_pharmacist'])) {
            ...
            if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin') {
                $update_stmt = $conn->prepare("UPDATE pharmacists SET is_disabled = 0 WHERE user_id = ?");
                $update_stmt->bind_param('s', $user_id);
                $update_result = $update_stmt->execute();
...
<html>
<head>
    <title>Disable/Enable Pharmacist Account</title>
</head>
<body>
    <?php if (isset($pharmacist)) : ?>
    <?php if ($pharmacist['is_disabled']) : ?>
    ...
    <?php else : ?>
        ...
        <form method="post" action="disable_pharmacist.php">
            <input type="submit" name="disable_pharmacist" value="Disable Account">
        </form>
    <?php endif; ?>

    <?php if (!$pharmacist['is_disabled'] && isset($_SESSION['role']) && $_SESSION['role'] === 'Admin') : ?>
        ...
        <form method="post" action="disable_pharmacist.php">
            <input type="submit" name="enable_pharmacist" value="Enable Account">
        </form>
    <?php endif; ?>
